...
def plot_totaltime(data, probs, figpath, normalized_value=1):
  ...
  plt.figure()
  m = -0.5
  approaches = ["no-resilient", "ckpt"]
  labels = {
    "no-resilient": "SART (No Resilience)",
    "ckpt": "SART with Checkpointing",
  }
  ...
  for approach in approaches:
    ...
    plt.bar(x + width*m, total/normalized_value, width, facecolor="none", edgecolor=appconf["color"], hatch="//", label=labels[approach])
...
if __name__ == "__main__":
  ...
  # python plot-runtime-conebeam.py data/execinfo-runtimes-cone-beam.json figures/runtime
  ...
  precalculated_runtimes = {
    0 : 111.719,
    0.001 : 224.55518999999887,
    0.01 : 1025.58042,
    0.1 : 26230004.62609352
  }

  ideal_exec_time = plotdata["baseline"]["time"]
  probs = [0, 0.001, 0.01, 0.1]
  for prob in probs:
    info = {}
    info["prob"] = prob
    info["time"] = precalculated_runtimes[prob]
    no_resilience["elapsed-time"].append(info)
  print(no_resilience)
  plotdata["approaches"]["no-resilient"] = no_resilience

  plot_totaltime(plotdata["approaches"], probs, figpath + "/elapsed-time-conebeam")
  # plot_overhead(plotdata["approaches"]["async"], probs, figpath + "/elapsed-time-breakdown-scalability", normalized_value=ideal_exec_time)
