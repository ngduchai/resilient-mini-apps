...
int main(int argc, char* argv[])
{
    ...
    std::string recon_method(argv[5]);
    const char* check_point_path = (argc == 7) ? argv[6] : nullptr;

    std::cout << "recon_method = " << recon_method << std::endl;
    ...
    for (int i = 0; i < num_outer_iter; i++)
    {
        ...
        // sirt(data_swap, dy, dt, dx, &center, theta, recon, ngridx, ngridy, num_iter);
        // mlem(data_swap, dy, dt, dx, &center, theta, recon, ngridx, ngridy, num_iter);
        recon_simple(recon_method, data_swap, dy, dt, dx, &center, theta, recon, ngridx, ngridy, num_iter);

        // write the reconstructed data to a file
        // Create the output file name
        std::ostringstream oss;
        if (check_point_path != nullptr) {
            oss << "cont_recon_" << recon_method << "_" << i << ".h5";
        } else {
            oss << "recon_" << recon_method << "_" << i << ".h5";
        }
