...
int main(int argc, char* argv[])
{
    ...
    if(argc != 9) {
        std::cerr << "Usage: " << argv[0] << " <filename> <center> <num_outer_iter> <num_iter> <beginning_sino> <num_sino> <skip_ratio> [veloc config]" << std::endl;
        return 1;
    }
    ...
    for (int i = 0; i < dt; ++i) {
        int c = std::rand() % 100 > skip_threshold;
